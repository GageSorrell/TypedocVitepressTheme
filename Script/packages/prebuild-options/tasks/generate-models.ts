...
import { type DocsConfig, SRC_PATH } from '@devtools/helpers';
...
export async function generateOptionsModels(docsConfig: DocsConfig) {
  const optionsConfig = await import(pathToFileURL(docsConfig.declarationsPath as string).href);
  ...
  if (manuallyValidatedOptions.length) {
    manuallyValidatedOptions.forEach((option: any) => {
      if (
        ...
        out.push(`import { ${option} } from './types/options.ts';`);
      }
    });
  }
...
function getType(
  ...
  const outType = `{${Object.keys(((option as unknown as any).defaultValue as unknown as any) || { })
    .map((key) => `'${key}'?: string;`)
    .join('')}}`;
