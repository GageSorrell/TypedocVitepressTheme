...
import type { DocsConfig } from "../../helpers/models.ts";
...
import { injectStringToFile } from './utils/inject-string-to-file.ts';
...
export async function generateOptionsDocs(docsConfig: DocsConfig) {
  ...
  if (docsConfig.presets) {
    ...
    const { presets }: any = await import(pathToFileURL(docsConfig.presetsPath as string).href);
...
\`\`\``);
  ...
  if (docsConfig.declarations) {
    const declarationsConfig: any = await import(
      pathToFileURL(docsConfig.declarationsPath as string).href
    );
    ...
    for (const [categoryName, options] of categories) {
      ...
      options.forEach((option) => {
        ...
        if (!option.help) {
          option.help = 'Configures the autogenerated VitePress sidebar.';
        //   throw new Error(
...
/**
 * pathToFileURLs the default value string for an option.
 */
...
/**
 * pathToFileURLs the example value string for an option.
 */
...
/**
 * pathToFileURLs the docs content directory for a given docs path.
 */
...
/**
 * pathToFileURLs the docs JSON directory path.
 */
